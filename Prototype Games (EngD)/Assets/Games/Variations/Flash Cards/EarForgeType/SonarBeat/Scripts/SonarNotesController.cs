...
public class SonarNotesController : MonoBehaviour
{
    ...
    private void EndGame(bool hasLostGame)
    {
        tickLine.degrees = 0;
        foreach (SonarNote spawnedNote in spawnedNotes)
        {
    ...
    //--Menu Controls--//
    public void SetNoteLengths(Dropdown dropdown)
    {
        string choice = dropdown.captionText.text;
        float newLengths = float.Parse(choice);
    ...
    public void SetCyclesToComplete(InputField value)
    {
        ...
        if (newCycles <= 0)
        {
            ...
            //some way to inform the user of infinite mode?
        }
        else
